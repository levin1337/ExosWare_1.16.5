...
import static ru.levinov.util.render.RenderUtil.Render2D.drawImage;
...
public class InventoryScreen extends DisplayEffectsScreen<PlayerContainer> implements IRecipeShownListener, IMinecraft {
    ...
    protected void init() {
        if (this.minecraft.playerController.isInCreativeMode()) {
            ...
            if (!ClientUtil.legitMode) {
                ...
                this.addButton(new Button(width / 2 - 50, height / 2 - 125, 100, 20, new StringTextComponent("Выкинуть всё!"), (button) ->
                {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (Managment.FUNCTION_MANAGER.optimization.state && Managment.FUNCTION_MANAGER.optimization.backroundinvenory.get()) {
            if (Managment.FUNCTION_MANAGER.swingAnimationFunction.state && Managment.FUNCTION_MANAGER.swingAnimationFunction.blurinv.get()) {
                GaussianBlur.startBlur();
                RenderUtil.Render2D.drawRoundedCorner(0,0,width,height,1, -1);
                GaussianBlur.endBlur(6,2f);
            } else {
    ...
    private void dropItems() {
        for (int i = 0; i < container.getInventory().size() && mc.currentScreen == this; ++i) {
