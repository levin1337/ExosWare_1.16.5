...
import ru.levinov.modules.impl.movement.TimerFunction;
...
public class ChatScreen extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.commandSuggestionHelper.onClick((double) ((int) mouseX), (double) ((int) mouseY), button)) {
            ...
            DragManager.draggables.values().forEach(dragging -> {
                if (dragging.getModule().state || dragging.getModule() instanceof TimerFunction) {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        boolean customChat = Managment.FUNCTION_MANAGER.streamMod.state;
        boolean reg1 = this.inputField.getText().startsWith("/reg");
        boolean reg2 = this.inputField.getText().startsWith("/l");
        boolean reg3 = this.inputField.getText().startsWith("/login");
        boolean reg4 = this.inputField.getText().startsWith("/warp");
        boolean reg5 = this.inputField.getText().startsWith("/tpa");
        ...
        if (customChat && (reg1 || reg2 || reg3 || Managment.FUNCTION_MANAGER.streamMod.warp.get() && reg4 || Managment.FUNCTION_MANAGER.streamMod.tpa.get() && reg5)) {
            int X = 2;
            ...
            RenderUtil.Render2D.drawRoundedRect((float)X, (float)Y, (float)width, (float)height, (float)radius, color);
            GaussianBlur.endBlur(15.0F, 1.0F);
        }
