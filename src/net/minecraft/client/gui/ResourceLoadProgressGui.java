...
import ru.levinov.util.BetterText;
...
public class ResourceLoadProgressGui extends LoadingGui {
    ...
    private BetterText text = new BetterText(Arrays.asList("думаю...", "запускаю...", "готовлю...", "жарю...", "почти все готово...", "ваша пицца будет у вас через 20 минут", "дождитесь", "серьезно?", "у тебя еще не загрузило?", "ну значит скоро все будет!", "смертникс пидор", "ну ладно пропустим."), 800);
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (f >= 1.0F) {
            ...
            int l = MathHelper.ceil((1.0F - MathHelper.clamp(f - 1.0F, 0.0F, 1.0F)) * 255.0F);
            RenderUtil.Render2D.drawRect(0, 0, width, height, ColorUtil.rgba(21, 25, 32, l));
            f2 = 1.0F - MathHelper.clamp(f - 1.0F, 0.0F, 1.0F);
        ...
        RenderUtil.Render2D.drawRect(0, 0, width, height, ColorUtil.rgba(21, 25, 32, 255 * f2));

        int color = (new Color(0, 0, 0, 90)).getRGB();
        RenderUtil.Render2D.drawImage(new ResourceLocation("client/images/logo" + ".png"), width / 2.23f, height / 2.8f, 120, 120, ColorUtil.getColorStyle(180));
        RenderUtil.Render2D.drawRoundedCorner(width / 3.3f, height / 1.4f,  width / 2.5f, 8.0F, 3.0F, color);
        RenderUtil.Render2D.drawRoundedCorner(width / 3.3f, height / 1.4f, width / 2.5F * progress, 8.0F, 3.0F, ColorUtil.rgba(0,0,255,255));

        Fonts.msBold[16].drawCenteredString(matrixStack, String.format("%.0f%%", this.progress * 100.0F), width / 2f, height / 1.3f, -1);
      //  Fonts.durman[23].drawCenteredString(matrixStack, Launch.nameClient, width / 2f, height / 2f, Color.BLUE.getRGB());
