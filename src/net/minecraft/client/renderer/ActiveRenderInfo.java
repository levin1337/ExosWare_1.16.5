...
public class ActiveRenderInfo
{
    ...
    public void update(IBlockReader worldIn, Entity renderViewEntity, boolean thirdPersonIn, boolean thirdPersonReverseIn, float partialTicks)
    {
        ...
        if (thirdPersonIn)
        {
            ...
            if (Managment.FUNCTION_MANAGER.babyBoy.state) {
                if (Managment.FUNCTION_MANAGER.cameraSpec.state) {
                    this.movePosition(-this.calcCameraDistance(Managment.FUNCTION_MANAGER.cameraSpec.zoom.getValue().floatValue()), -0.5D, Managment.FUNCTION_MANAGER.cameraSpec.scroll.getValue().floatValue());
                } else {
                    this.movePosition(-this.calcCameraDistance(2.0D), -0.5D, 0.0D);
                }
            } else {
                if (Managment.FUNCTION_MANAGER.cameraSpec.state) {
                    this.movePosition(-this.calcCameraDistance(Managment.FUNCTION_MANAGER.cameraSpec.zoom.getValue().floatValue()), 0.0D, Managment.FUNCTION_MANAGER.cameraSpec.scroll.getValue().floatValue());
                } else {
    ...
    private double calcCameraDistance(double startingDistance) {
        ...
        interpolatedDistance = AnimationMath.fast(interpolatedDistance, (float) startingDistance, 10000);
