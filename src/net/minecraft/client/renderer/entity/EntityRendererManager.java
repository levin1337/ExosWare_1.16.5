...
public class EntityRendererManager {
    ...
    private void renderBoundingBox(MatrixStack matrixStackIn, IVertexBuilder bufferIn, Entity entityIn, float red, float green, float blue) {
        ...
        if (Managment.FUNCTION_MANAGER.xbox.state) {
            if (Managment.FUNCTION_MANAGER.xbox.mode.is("Тематический")) {
                final float[] colors = RenderUtil.IntColor.rgb(Managment.STYLE_MANAGER.getCurrentStyle().colors[1]);
                red = colors[0];
                green = colors[1];
                blue = colors[2];
                WorldRenderer.drawBoundingBox(matrixStackIn, bufferIn, axisalignedbb, red, green, blue, Managment.FUNCTION_MANAGER.xbox.alpha.getValue().floatValue());
            }
            if (Managment.FUNCTION_MANAGER.xbox.mode.is("Свой")) {
                red2 = Managment.FUNCTION_MANAGER.xbox.color.getColor().getRed();
                green2 = Managment.FUNCTION_MANAGER.xbox.color.getColor().getGreen();
                blue2 = Managment.FUNCTION_MANAGER.xbox.color.getColor().getBlue();
                WorldRenderer.drawBoundingBox(matrixStackIn, bufferIn, axisalignedbb, red2, green2, blue2, Managment.FUNCTION_MANAGER.xbox.alpha.getValue().floatValue());
            }
        } else {
            WorldRenderer.drawBoundingBox(matrixStackIn, bufferIn, axisalignedbb, red, green, blue, 1);
        }
    }
