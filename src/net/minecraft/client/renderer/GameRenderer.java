...
import ru.levinov.util.glu.Project;
...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    public double getFOVModifier(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            factor = AnimationMath.fast(factor, flag ? 4 : 1, 10000);
            d0 /= factor;
    ...
    public void hurtCameraEffect(MatrixStack matrixStackIn, float partialTicks) {
        if (Managment.FUNCTION_MANAGER.swingAnimationFunction.state && Managment.FUNCTION_MANAGER.swingAnimationFunction.damage.get()) {
            if (this.mc.getRenderViewEntity() instanceof LivingEntity) {
                ...
                // Добавляем эффект отягивания экрана
                float scale = 2.0F + f * 0.1F; // Увеличиваем масштаб в зависимости от фракции урона
                matrixStackIn.scale(scale, scale, scale); // Применяем масштабирование

                // Смещение камеры назад для эффекта отягивания
                matrixStackIn.translate(0.0F, 0.0F, -f * 0.5F); // Смещаем камеру назад

                // Эффект сдвижения вниз
                float downwardShift = f * 0.5F; // Сдвиг вниз в зависимости от фракции урона
                matrixStackIn.translate(0.0F, -downwardShift, 0.0F); // Смещаем камеру вниз
            }
        } else {
            if (this.mc.getRenderViewEntity() instanceof LivingEntity) {
                LivingEntity livingentity = (LivingEntity) this.mc.getRenderViewEntity();
    ...
    public void applyBobbing(MatrixStack matrixStackIn, float partialTicks) {
        if (Managment.FUNCTION_MANAGER.swingAnimationFunction.state && Managment.FUNCTION_MANAGER.swingAnimationFunction.run.get()) {
            if (this.mc.getRenderViewEntity() instanceof PlayerEntity) {
                ...
                // Добавляем плавные колебания
                float bobbingIntensity = 0.05F; // Интенсивность качания
                float bobbingSpeed = 2.0F; // Скорость качания
                float bobbingOffsetY = MathHelper.sin(f1 * bobbingSpeed) * bobbingIntensity; // Вертикальное колебание
                float bobbingOffsetX = MathHelper.sin(f1 * bobbingSpeed + (float) Math.PI / 2) * bobbingIntensity; // Горизонтальное колебание

                // Плавные смещения камеры
                matrixStackIn.translate(
                        (double) (bobbingOffsetX), // Горизонтальное смещение
                        (double) (-Math.abs(MathHelper.cos(f1 * (float) Math.PI) * f2) + bobbingOffsetY), // Вертикальное смещение
                        0.0D
                );

                // Повороты камеры с использованием синусоидальных функций для плавности
                float rotationIntensity = 3.0F; // Интенсивность вращения
                matrixStackIn.rotate(Vector3f.ZP.rotationDegrees(MathHelper.sin(f1 * (float) Math.PI) * f2 * rotationIntensity));
                matrixStackIn.rotate(Vector3f.XP.rotationDegrees(Math.abs(MathHelper.cos(f1 * (float) Math.PI - 0.2F) * f2) * 5.0F + bobbingOffsetY * 2.0F));
            }
        } else {
            if (this.mc.getRenderViewEntity() instanceof PlayerEntity) {
                PlayerEntity playerentity = (PlayerEntity) this.mc.getRenderViewEntity();
    ...
    public Matrix4f getProjectionMatrix(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFovSetting) {
        ...
        if (Managment.FUNCTION_MANAGER.aspectRatio.state) {

            matrixstack.getLast().getMatrix().mul(Matrix4f.perspective(this.getFOVModifier(activeRenderInfoIn, partialTicks, useFovSetting), Managment.FUNCTION_MANAGER.aspectRatio.isState() ? (float)Managment.FUNCTION_MANAGER.aspectRatio.scroll.getValue().floatValue() : (float)this.mc.getMainWindow().getFramebufferWidth() / (float)this.mc.getMainWindow().getFramebufferHeight(), 0.05f, this.clipDistance));
    ...
    public void setupOverlayRendering(int scale) {
        ...
        GlStateManager.ortho(0.0D, mainWindow.getScaledWidth(), mainWindow.getScaledHeight(), 0.0D, 1000.0D, 3000.0D);
    ...
    public void renderWorld(float partialTicks, long finishTimeNano, MatrixStack matrixStackIn) {
        ...
        // RENDER
        if (Managment.FUNCTION_MANAGER.noRenderFunction.effectcamera.get()) {
        } else {
        ...
        EventManager.call(new EventRender2(partialTicks, matrixStackIn, sr, EventRender2.Type.WORLD3D, matrix4f, activerenderinfo));
        // Particles
        EventManager.call(new EventRenderWorld(matrixStackIn,mc.gameRenderer.getActiveRenderInfo(),partialTicks));
