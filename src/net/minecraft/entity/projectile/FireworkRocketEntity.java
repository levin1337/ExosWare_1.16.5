        ...
        if (this.isAttachedToEntity()) {
            ...
            if (this.boostedEntity != null) {
                if (this.boostedEntity.isElytraFlying()) {
                    ...
                    double MAX_ANGLE2 = 2.0;
                    if (Managment.FUNCTION_MANAGER.elytraBoost.state) {
                        if (Managment.FUNCTION_MANAGER.elytraBoost.mode.is("ReallyWorld")) {
                            speed = 1.620F;
                        }
                        if (Managment.FUNCTION_MANAGER.elytraBoost.mode.is("BravoHvH")) {
                            if (Math.abs(Minecraft.getInstance().player.rotationPitchHead) > MAX_ANGLE2 || Math.abs(Minecraft.getInstance().player.rotationYawHead) > MAX_ANGLE2) {
                                speed = 1.820F;
                            } else {
                                speed = 1.780F;
                            }
                        }
                        if (Managment.FUNCTION_MANAGER.elytraBoost.mode.is("Свой")) {
                            speed = Managment.FUNCTION_MANAGER.elytraBoost.speed.getValue().floatValue();
                        }
                        if (Managment.FUNCTION_MANAGER.auraFunction.state && Managment.FUNCTION_MANAGER.auraFunction.settings.get(4) && boostedEntity instanceof ClientPlayerEntity) {
                            Vector3d vector3d = getVectorForRotation(Managment.FUNCTION_MANAGER.auraFunction.rotate.y, Managment.FUNCTION_MANAGER.auraFunction.rotate.x);
                            Vector3d vector3d1 = boostedEntity.getMotion();
                            boostedEntity.setMotion(vector3d1.add(vector3d.x * 0.1 + (vector3d.x * (double)speed - vector3d1.x) * 0.5, vector3d.y * 0.1 + (vector3d.y * (double)speed - vector3d1.y) * 0.5, vector3d.z * 0.1 + (vector3d.z * (double)speed - vector3d1.z) * 0.5));
                            if (Managment.FUNCTION_MANAGER.elytraBoost.doble.get()) {
                                boostedEntity.setMotion(vector3d1.add(vector3d.x * 0.1 + (vector3d.x * (double) speed - vector3d1.x) * 0.5, vector3d.y * 0.1 + (vector3d.y * (double) speed - vector3d1.y) * 0.5, vector3d.z * 0.1 + (vector3d.z * (double) speed - vector3d1.z) * 0.5));
                                boostedEntity.setMotion(vector3d1.add(vector3d.x * 0.1 + (vector3d.x * (double) speed - vector3d1.x) * 0.5, vector3d.y * 0.1 + (vector3d.y * (double) speed - vector3d1.y) * 0.5, vector3d.z * 0.1 + (vector3d.z * (double) speed - vector3d1.z) * 0.5));
                            } else {
                            ...
                            boostedEntity.setMotion(vector3d1.add(vector3d.x * 0.1 + (vector3d.x * (double)speed - vector3d1.x) * 0.5, vector3d.y * 0.1 + (vector3d.y * (double)speed - vector3d1.y) * 0.4, vector3d.z * 0.1 + (vector3d.z * (double)speed - vector3d1.z) * 0.5));
                            //default speed
                         //   boostedEntity.setMotion(vector3d1.add(vector3d.x * 0.1D + (vector3d.x * 1.5D - vector3d1.x) * 0.5D, vector3d.y * 0.1D + (vector3d.y * 1.5D - vector3d1.y) * 0.5D, vector3d.z * 0.1D + (vector3d.z * 1.5D - vector3d1.z) * 0.5D));
                        }
