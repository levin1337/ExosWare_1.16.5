...
import ru.levinov.events.impl.player.EventDamage;
import ru.levinov.events.impl.player.EventJump;
import ru.levinov.events.impl.player.EventStrafe;
import ru.levinov.events.impl.player.EventTravel;
...
public abstract class LivingEntity extends Entity {
    ...
    protected void jump() {
        ...
        if (Managment.FUNCTION_MANAGER.crystalAura.check()) {
            if (this.isSprinting()) {
                float f1 = Managment.FUNCTION_MANAGER.crystalAura.rotate.x * ((float) Math.PI / 180F);
                this.setMotion(this.getMotion().add((double) (-MathHelper.sin(f1) * 0.2F), 0.0D, (double) (MathHelper.cos(f1) * 0.2F)));
            }
            this.isAirBorne = true;
            return;
    ...
    public void travel(Vector3d travelVector) {
        if (this.isServerWorld() || this.canPassengerSteer()) {
            ...
            if (this.isInWater() && this.func_241208_cS_() && !this.func_230285_a_(fluidstate.getFluid())) {
                ...
                Vector3d vector3d1 = getLookVec();
                // SPRINT
                if (Managment.FUNCTION_MANAGER.auraFunction.state && Managment.FUNCTION_MANAGER.auraFunction.settings.get(4) && this instanceof ClientPlayerEntity) {
