...
import static ru.levinov.util.render.OutlineUtils.setupBuffer;
import static ru.levinov.util.render.ShaderUtil.calculateGaussianValue;
...
public class BloomHelper {
    ...
    public static void drawfps(int radius, float exp, boolean fill) {
        ...
        outFrameBuffer.bindFramebuffer(true);
        bloom.attach();
        bloom.setUniformf("radius", new float[]{(float)radius});
        bloom.setUniformf("exposure", new float[]{exp});
        bloom.setUniform("textureIn", new int[]{0});
        bloom.setUniform("textureToCheck", new int[]{20});
        bloom.setUniform("avoidTexture", new int[]{fill ? 1 : 0});
        FloatBuffer weightBuffer = BufferUtils.createFloatBuffer(256);

        for(int i = 0; i <= radius; ++i) {
            weightBuffer.put(ShaderUtil.calculateGaussianValue((float)i, (float)(radius / 2)));
        }

        weightBuffer.rewind();
        RenderSystem.glUniform1(bloom.getUniform("weights"), weightBuffer);
        bloom.setUniformf("texelSize", new float[]{1.0F / (float)Minecraft.getInstance().getMainWindow().getWidth(), 1.0F / (float)Minecraft.getInstance().getMainWindow().getHeight()});
        bloom.setUniformf("direction", new float[]{1.0F, 0.0F});
        GlStateManager.enableBlend();
        ...
        bloom.setUniformf("direction", new float[]{0.0F, 1.0F});
        outFrameBuffer.bindFramebufferTexture();
