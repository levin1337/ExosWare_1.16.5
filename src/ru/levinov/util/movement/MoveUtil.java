...
public class MoveUtil implements IMinecraft {
    ...
    public static void boostElytra(float speed) {
        if (!MoveUtil.isMoving()) {
            return;
        }
        double yaw = MoveUtil.getAllDirection();
        mc.player.motion.x = -Math.sin(yaw) * (double) speed;
        mc.player.motion.z = Math.cos(yaw) * (double) speed;
    }
    ...
    public static double[] forward(double d) {
        ...
        float var10000 = mc.player.prevRotationYaw;
        float var10001 = mc.player.rotationYaw;
        float f3 = var10000 + (var10001 - mc.player.prevRotationYaw) * mc.getRenderPartialTicks();
        if (f != 0.0F) {
            if (f2 > 0.0F) {
