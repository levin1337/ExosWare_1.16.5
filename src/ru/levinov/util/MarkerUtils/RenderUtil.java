...
public class RenderUtil implements IMinecraft {
    ...
    public static Color twocolor(double delay) {
        Color color1 = Color.RED;       // Красный
        Color color2 = Color.BLUE;      // Синий
        Color color3 = Color.YELLOW;    // Желтый
        Color color4 = Color.GREEN;     // Зеленый
        Color color5 = Color.CYAN;      // Голубой

        // Определяем количество цветов
        Color[] colors = {color1, color2, color3, color4, color5};
        int numColors = colors.length;

        // Нормализуем delay для работы с цветами
        double normalizedDelay = delay % numColors; // Приводим к диапазону [0, numColors)
        int index1 = (int) normalizedDelay; // Индекс первого цвета
        int index2 = (index1 + 1) % numColors; // Индекс второго цвета

        // Вычисляем интерполяцию между двумя цветами
        double blendFactor = normalizedDelay - index1; // Фактор смешивания (от 0 до 1)
    ...
    public static void vegaRender(Runnable run) {
        ...
        GL11.glPopMatrix();
        //рендер на rendersystem, но он хуже и тусклее
