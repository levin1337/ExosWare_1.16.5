...
import ru.levinov.modules.settings.imp.ThemSetting;
...
public class ThemObject extends Object {
    ...
    public void draw(MatrixStack stack, int mouseX, int mouseY) {
        ...
        height -= 1;
        Fonts.durman[13].drawString(stack, set.getName(), x + 12, y + height / 2f - 5.5f, ColorUtil.rgba(240, 240, 240, 255));
        ...
        for (String mode : set.modes) {
            ...
            int color = ColorUtil.rgba(255, 255, 255, 255);  // Default color
            ...
            if (styleColors.containsKey(mode)) {
                for (Style style : Managment.STYLE_MANAGER.styles) {
                    if (style.name.equals(mode)) {
                        if (mode.equals("13")) {
                            color = ColorUtil.astolfo(10, 1, 0.5F, 1.0F, 1.0F);
                        } else if (mode.equals("14")) {
                            color = ColorUtil.rgba(255, 255, 255, 255);
                        } else {
            ...
            if (set.getIndex() == i) {
                ...
                RenderUtil.Render2D.drawRoundedRect(x + 13f + finalOffset, y + 9f + finalOffsetY, 9.5f, 9.5f, 2f, color);
                RenderUtil.Render2D.drawRoundOutline(x + 13f + finalOffset, y + 9f + finalOffsetY, 9.5f, 9.5f, 2, 0f, ColorUtil.rgba(25, 26, 33, 0), new Vector4i(new Color(120, 120, 120, 192).getRGB(), new Color(120, 120, 120, 192).getRGB(), new Color(120, 120, 120, 192).getRGB(), new Color(120, 120, 120, 192).getRGB()));
            } else {
                RenderUtil.Render2D.drawRoundedRect(x + 13f + offset, y + 9f + offsetY, 9.5f, 9.5f, 2f, color);
                RenderUtil.Render2D.drawRoundOutline(x + 13f + offset, y + 9f + offsetY, 9.5f, 9.5f, 2, 0f, ColorUtil.rgba(25, 26, 33, 0), new Vector4i(new Color(73, 73, 73, 128).getRGB(), new Color(73, 73, 73, 128).getRGB(), new Color(73, 73, 73, 128).getRGB(), new Color(73, 73, 73, 128).getRGB()));
            }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (object.module.expanded) {
            for (String mode : set.modes) {
                ...
                if (RenderUtil.isInRegion(mouseX, mouseY, x + 13f + offset, y + 9f + offsetY, 9.5f, 9.5f)) {
                    set.setIndex(i);
                    closeGui();
                }
